...
public class SimpleSpanCodecTest {
  ...
  SimpleSpan span = SimpleSpan.builder()
  ...
  @Test public void spanRoundTrip() throws IOException {
    byte[] bytes = SimpleSpanCodec.writeSpan(span);
    assertThat(SimpleSpanCodec.readSpan(bytes))
      .isEqualTo(span);
  }

  @Test public void sizeInBytes() throws IOException {
    assertThat(SimpleSpanCodec.SPAN_WRITER.sizeInBytes(span))
      .isEqualTo(SimpleSpanCodec.writeSpan(span).length);
  }
  ...
  @Test public void specialCharsInJson() throws IOException {
    ...
    SimpleSpan worstSpanInTheWorld = SimpleSpan.builder().traceId(1L).id(1L)
      // name is terrible
    ...
    byte[] bytes = SimpleSpanCodec.writeSpan(worstSpanInTheWorld);
    assertThat(SimpleSpanCodec.readSpan(bytes))
  ...
  @Test public void decentErrorMessageOnEmptyInput_span() throws IOException {
    ...
    thrown.expectMessage("Empty input reading SimpleSpan");

    SimpleSpanCodec.readSpan(new byte[0]);
  }

  @Test public void decentErrorMessageOnEmptyInput_spans() throws IOException {
    thrown.expect(IllegalArgumentException.class);
    thrown.expectMessage("Empty input reading List<SimpleSpan>");

    SimpleSpanCodec.readSpans(new byte[0]);
  }

  @Test public void decentErrorMessageOnMalformedInput_span() throws IOException {
    thrown.expect(IllegalArgumentException.class);
    thrown.expectMessage("Malformed reading SimpleSpan from ");

    SimpleSpanCodec.readSpan(new byte[] {'h', 'e', 'l', 'l', 'o'});
  }
  ...
  @Test public void decentErrorMessageOnMalformedInput_spans() throws IOException {
    thrown.expect(IllegalArgumentException.class);
    thrown.expectMessage("Malformed reading List<SimpleSpan> from ");

    SimpleSpanCodec.readSpans(new byte[] {'h', 'e', 'l', 'l', 'o'});
  }

  @Test public void spansRoundTrip() throws IOException {
    List<SimpleSpan> tenClientSpans = Collections.nCopies(10, span);

    byte[] bytes = SimpleSpanCodec.writeSpans(tenClientSpans);
    assertThat(SimpleSpanCodec.readSpans(bytes))
      .isEqualTo(tenClientSpans);
  }

  @Test public void writesTraceIdHighIntoTraceIdField() {
    SimpleSpan with128BitTraceId = SimpleSpan.builder()
      .traceIdHigh(Util.lowerHexToUnsignedLong("48485a3953bb6124"))
      .traceId(Util.lowerHexToUnsignedLong("6b221d5bc9e6496c"))
      .localEndpoint(frontend)
      .id(1).name("").build();

    assertThat(new String(SimpleSpanCodec.writeSpan(with128BitTraceId), Util.UTF_8))
  ...
  @Test public void readsTraceIdHighFromTraceIdField() {
    ...
    assertThat(SimpleSpanCodec.readSpan(with128BitTraceId))
      .isEqualTo(SimpleSpanCodec.readSpan(withLower64bitsTraceId).toBuilder()
  ...
  @Test public void ignoreNull_parentId() {
    ...
    SimpleSpanCodec.readSpan(json.getBytes(UTF_8));
  }
  ...
  @Test public void niceErrorOnNull_localEndpoint() {
    thrown.expect(IllegalArgumentException.class);
    thrown.expectMessage("Null localEndpoint reading SimpleSpan from json");
